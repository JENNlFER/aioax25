...
- `retransmit_count`, `retransmit_timeout_base`, `retransmit_timeout_rand`,
  ...
  is computed as `timeout = retransmit_timeout_base + (random.random() * retransmit_timeout_rand)`, and a retry counter is initialised to
...
messages on the interface they were received from _ONLY_. (i.e. if you connect
a 2m interface and a HF interface, it will _NOT_ digipeat from HF to 2m).
...
If you have a _lot_ of digipeaters in close proximity (say about 6) and there's
...
a message because the digipeats come more than 30 seconds _after_ the original.
